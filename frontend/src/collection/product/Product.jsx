...
const Product = () => {
    ...
    useEffect(() => {
        const item = cataloges?.find((item) => item.path === typeCatalog.path)?.name
        setCatalog(item)
    }, [cataloges])
    console.log(catalog)
        ...
        <>
        {
            (cataloges.length > 0) ? (
                ...
                <div className="container__producthome--item col-lg-12 col-md-12 col-sm-12 col-12 row">
                        <h3 style={{color: "#000000", fontWeight: "600"}}>{catalog}</h3>
                        {
                                Products?.map((Product, idx) => {
                                    if ( Product.catalog === catalog || Product.catalogParent === catalog){
                                        return (
                                            <>
                                            <div className="container__producthome--child col-lg-4 col-md-4 col-sm-6 col-6 pt-3 px-2 " key={idx}>
